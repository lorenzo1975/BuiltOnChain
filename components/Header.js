...
export default function Header() {
    ...
    const styles = {
        navClass: 'flex-col md:flex md:flex-row items-center justify-center flex-grow px-5 py-3 md:pb-0 '
    }
    const onToggleNav = () => {
        setNavShow((open) => {
          return open = !open
...
<div className="w-full text-yellow-400 backdrop-filter backdrop-blur-3xl">
    <div className="flex flex-col w-full px-4 mx-auto md:items-center md:justify-between md:flex-row md:px-6 lg:px-32">
        <div className="flex flex-row items-center justify-between p-4">
            <Logo />
            <button className="md:hidden focus:outline-none" onClick={onToggleNav} aria-label="hidden">
        ...
        <nav
        className={`${styles.navClass} ${!navShow ? 'flex' : 'hidden'}`}>
